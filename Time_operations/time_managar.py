...
def parse_input(input_text):
    ...
    time_regex = r'(\d{1,2}:\d{2}\s?(?:A\.?M\.?|P\.?M\.?|am|pm|AM|PM))'
...
def input_manage(input_text):
    ...
    if output != "No valid time found in input":
        save_to_file(output, time, r'C:\Users\dell\OneDrive\Desktop\Vivie_v1\schedule.txt')
        print("Sechedule_Data_Saved")
    else:
        print(output)
...
def input_manage_Alam(input_text):
    ...
    if time != "No valid time found in input":
        save_to_Alarmfile(time, r'C:\Users\dell\OneDrive\Desktop\Vivie_v1\alarm_data.txt')
        print("Sechedule_Data_Saved")
    else:
        print("pass")